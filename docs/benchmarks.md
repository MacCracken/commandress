...
`cmdrs` runs once per shell prompt redraw. The render must be fast enough to be **invisible** at typing speed, and predictable enough that adding segments doesn't blow the envelope. This document is the v1.0 finalised view of what we measure, the budgets we hold ourselves to, and the numbers we hit.
...
| All segments combined | ≤ 2.5 ms | ~7 µs default set + ~7 µs cached vcs |
| Render + final write | ≤ 250 µs | bundled into `render_prompt` ~10 µs |
...
Numbers from the v0.9.0 release run (Linux 7.0.5-arch1-1, x86_64, dev host, cache cleared between runs). Captured by `cyrius bench tests/commandress.bcyr`; historical series in [`benchmarks/history.csv`](benchmarks/history.csv).
...
| `vcs_render` (**cached, 1 s TTL**) | ~7 µs hot avg | Cold path: fork + exec + parse ≈ 1.8 ms. Cache hit: stat + read + reset mtime. The headline M6 win — 70× faster on the cached path. |

Across the default `["cwd", "vcs", "exit"]` segment set, **`render_prompt` is ~10 µs avg** — **0.2 % of the 5 ms budget consumed**.
...
| Cache warm | **~0.5 ms** | Fork + exec + cache hit (stat + read) + write to stdout |

Both are well under the perceptible-latency threshold (~50 ms for keystroke-to-screen).

## Trends

The CSV at [`benchmarks/history.csv`](benchmarks/history.csv) accumulates one row per (release × bench) since v0.6.1, normalised to nanoseconds. Plotting `avg_ns` per `name` against `version` shows:

- **`vcs_render`**: 4.6 ms (v0.5.0) → 67 µs (v0.7.0+). M6 cache landed.
- **`render_prompt`**: 2 µs (v0.5.0) → 3 µs (v0.6.0 — adds SGR wrap overhead) → 9–10 µs (v0.7.0+ — vcs now in the default set, still cached).
